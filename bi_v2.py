...
# ==================== PROTEÇÃO POR SENHA (CÓDIGO ANTIGO - COMENTADO) ====================
# # ==================== PROTEÇÃO POR SENHA ====================
...
# Configuração via Streamlit (na sidebar)
with st.sidebar:
    ...
    if senha_env:
        ...
        st.caption(f"Usando: DASH_SENHA")
    else:
...
with tab_gerenciamento:
    ...
    if not st.session_state['gerenciamento_autenticado']:
        ...
        with col_senha:
            senha_input = st.text_input('Digite a senha para acessar:', type='password', key='senha_gerenciamento')
        with col_btn:
            st.markdown('<br>', unsafe_allow_html=True)
            if st.button('🔓 Entrar', use_container_width=True):
                # Verifica primeiro se existe senha customizada, depois variável de ambiente, por último o fallback
                senha_correta = st.session_state.get('senha_customizada') or os.environ.get('DASH_SENHA', 'admin123')
        ...
        if st.session_state.get('senha_customizada'):
            st.caption("🔒 Usando senha customizada definida via interface")
        elif os.environ.get('DASH_SENHA'):
            st.caption("🔒 Usando variável de ambiente DASH_SENHA")
        else:
            st.caption("🔒 Usando senha padrão (admin123)")
    else:
        # Usuário autenticado - mostrar conteúdo da área de gerenciamento
...
Poderia verificar o envio na origem?
        ...
        with tab_historico:
            ...
            if not historico:
                ...
                st.divider()
                with st.expander('📥 Download do Banco de Dados', expanded=False):
                    st.warning('⚠️ Esta funcionalidade permite baixar o banco completo. Use apenas para backup!')
                    db_path = os.path.join(os.path.dirname(__file__), "integracoes.db")
                    if os.path.exists(db_path):
                        with open(db_path, "rb") as f:
                            st.download_button(
                                label="💾 Baixar integracoes.db",
                                data=f,
                        ...
                        st.error('❌ Arquivo de banco não encontrado!')

# ==================== RODAPÉ ====================
st.divider()
st.caption('BI de Integrações v2.0 - Sistema de gestão de integrações')
